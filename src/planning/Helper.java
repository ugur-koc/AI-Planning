...
public class Helper {
	public static boolean satifies(Variable[] variables, State state) {
		for (Variable aVariable : variables)
			if (!state.getVariables().contains(aVariable)) return false;
		return true;
	}
	...
	private static State createState(String line) {
		...
		for (String string : variableArr) {
			...
			variables.add(new Variable(name, parameters, null, split[1], null)); // TODO
		}
	...
	private static Action createAction(String line, String preLine, String effLine) {
		...
		return new Action(name, parameters, preconditions, effects);
	}
	...
	private static Variable createVariable(B b, String line, String domLine) {
		...
		return new Variable(name, parameters, b.getPlanningObject(domLine.split("=")[1]).getElements(), null, null);// TODO
	}
}